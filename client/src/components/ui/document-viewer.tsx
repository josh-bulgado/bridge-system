...
export function DocumentViewer({
  ...
  const [, setIsLoadingSignedUrl] = useState(false);
  ...
  const getDocumentUrl = () => {
    ...
    if (publicId) {
      const cloudName =
        import.meta.env.VITE_CLOUDINARY_CLOUD_NAME || "drc4nalg0";
      ...
      if (isPdf || fileType === "application/pdf") {
        ...
        const pdfPublicId = publicId.endsWith(".pdf")
          ? publicId
          : `${publicId}.pdf`;
  ...
  if (!documentUrl) {
    return (
      <div
        className={cn(
          "bg-muted/30 rounded-lg border p-6 text-center",
          className,
        )}
      >
        <AlertTriangle className="mx-auto mb-2 h-10 w-10 text-amber-500" />
        <p className="text-muted-foreground text-sm">Document not available</p>
      </div>
    );
  }
  ...
  return (
    <div className={cn("space-y-2", className)}>
      ...
      {isPdf ? (
        ...
        <div className="group bg-muted/30 relative overflow-hidden rounded-lg border">
          {pdfPreviewUrl && !imageError ? (
            <>
              <img
                ...
                onError={(e) => {
                  console.log(
                    "PDF preview not available (likely untrusted account):",
                    { publicId, pdfPreviewUrl, e },
                  );
                  setImageError(true);
                }}
              />
            ...
            <div
              className="hover:bg-muted/50 cursor-pointer p-8 text-center transition-colors"
              onClick={openFullSize}
            >
        ...
        <div className="bg-muted/30 relative rounded-lg border p-6 text-center">
      ...
      <Dialog open={isPdfDialogOpen} onOpenChange={setIsPdfDialogOpen}>
        <DialogContent className="max-h-[90vh] max-w-4xl overflow-hidden">
          ...
          {signedPdfUrl && <PdfViewer url={signedPdfUrl} title={title} />}
        </DialogContent>
      </Dialog>

...
export function useEmailAvailability(
  ...
  const debounceTimer = useRef<number | null>(null); // Explicitly typed as number
  ...
  useEffect(() => {
    ...
    if (timeSinceLastRequest < 60000) {
      ...
      if (requestCount.current > 8) {
        // Client-side limit slightly lower than server
        setIsChecking(false);
      ...
      async () => {
        try {
          ...
          const timeoutPromise = new Promise((_, reject) => {
            setTimeout(() => {
              console.log(
                `[EMAIL CHECK] ⏱️ Request timed out after 30 seconds`,
              );
          ...
          console.log(
            `[EMAIL CHECK] ✅ Completed in ${duration}ms | Available: ${result.available}`,
          );

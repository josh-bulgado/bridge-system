  ...
  const handleFiles = useCallback(
    async (files: FileList) => {
      ...
      if (uploadedFiles.length + fileArray.length > maxFiles) {
        toast.error(
          `Maximum ${maxFiles} files allowed. You already have ${uploadedFiles.length} file(s) uploaded.`,
        );
        return;
      }
      ...
      if (invalidFiles.length > 0) {
        ...
        console.error(
          "Invalid file types:",
          invalidFiles.map((f) => `${f.name} (${f.type})`),
        );
        return;
      }
      ...
      {uploadedFiles.length < maxFiles && (
        ...
        >
          ...
          <div className="flex flex-col items-center gap-2">
            {uploading ? (
              <IconLoader2 className="text-muted-foreground h-10 w-10 animate-spin" />
            ) : (
            ...
            <div className="space-y-1">
              ...
              <p className="text-primary text-xs font-medium">
                You can upload up to {maxFiles - uploadedFiles.length} more
                file(s)
              </p>
            </div>
          </div>
            ...
            {uploadedFiles.map((file, index) => (
              ...
              >
                <div className="flex min-w-0 flex-1 items-center gap-3">
                  <IconFile className="text-muted-foreground h-5 w-5 shrink-0" />
